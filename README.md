# Sociale Kaart Bonaire

> Deze statische MVP bevat startdata. Organisatienamen, aanbod en activiteiten moeten door de stagiair/opdrachtgever worden gecontroleerd voordat publicatie plaatsvindt.

Statische MVP voor bespreking met stagiair/opdrachtgever.
...
- `tools/optimize_assets.py` - maakt WebP-versies van brand-assets en thema-iconen

## Assets optimaliseren
Originele PNG-bestanden blijven staan. Webversies maak je opnieuw met:

```bash
python tools/optimize_assets.py
```

De gegenereerde bestanden komen in `assets/theme-icons/web/` en `assets/brand/web/`.

## Doel
Deze MVP is bedoeld als voorzet. Niet als definitieve technische keuze. De structuur is later te vertalen naar Laravel, WordPress of een ander CMS/platform.


## Site vullen

De MVP gebruikt JSON-bestanden in de map `data/`:
...
Voor een snelle werkversie kun je deze bestanden handmatig aanpassen. Voor een echte inventarisatie is het slimmer om eerst het Excel-template `datacollectie_template_sociale_kaart_BES.xlsx` te laten invullen en daarna de data om te zetten naar JSON.
